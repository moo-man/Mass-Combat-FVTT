Hooks.on("renderSidebarTab", (app, html) => {
  ...
  import {hello} from "./combat-resolution/dnd5e.js"
  ...
  class MassCombatTool extends Application {
        ...
        {
            const START_Y = 75;
            const INCREMENT_Y = 50;
            const WIDTH = 175;

            let data = super.getData();
            data.combat = this.combat;

            if (this.combat.targets.friendly)
            {
                data.friendlyLines = [];
                data.hostileLines = [];
                let y = START_Y;
                for (let friendlyTargets of this.combat.targets.friendly )
                {
                    try {
                        let targetY = (this.combat.hostile.findIndex(h => h._id == friendlyTargets.target._id) * INCREMENT_Y) + START_Y
                        data.friendlyLines.push({yFriendly : y, yHostile : targetY})
                        y+=INCREMENT_Y
                    }
                    catch (e) {
                        console.log("Couldn't find target for " +  friendlyTargets.token.name)
                    }

                }

                y = START_Y+5;
                for (let hostileTargets of this.combat.targets.hostile )
                {
                    try {
                    let targetY = (this.combat.friendly.findIndex(f => f._id == hostileTargets.target._id) * INCREMENT_Y) + START_Y+5
                    data.hostileLines.push({yHostile : y, yFriendly : targetY})
                    y+=INCREMENT_Y
                    }
                    catch (e) {
                        console.log("Couldn't find target for " +  hostileTargets.token.name)
                    }
                }
            }
